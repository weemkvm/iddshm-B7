...
static void RegisterTrayIcon(void)
{
  ...
  if (!app.iconData.cbSize)
  {
    ...
    strncpy(app.iconData.szTip, "IDDShm Host", sizeof(app.iconData.szTip));
...
static void populateSystemLogDirectory(void)
{
  ...
  {
    if (!PathCombineA(app.systemLogDir, programData, "IDDShm Host"))
      goto fail;
...
void boostPriority(void)
{
        ...
        D3DKMT_SCHEDULINGPRIORITYCLASS_REALTIME)))
  {
    ...
    DEBUG_INFO("IDDShmHost.exe InstallService");
  }
